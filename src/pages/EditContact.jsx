...
export default function EditContact() {
    ...
    useEffect(() => {
        const fetchContact = async () => {
            try {
                const res = await axios.get(`/api/contacts/${customer_id}`, {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setLoading(true);
            await axios.put(`/api/contacts/${customer_id}`, { name, phone, email }, {
    ...
    return (
        <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", fontFamily: "'Segoe UI', sans-serif" }}>
            <div style={{ background: "white",borderRadius: "30px", padding: "50px 40px", width: "100%", maxWidth: "400px", boxShadow: "0 30px 100px rgba(0,0,0,0.3)" }}>
               ...
               <h2 style={{ margin: "0 0 30px", color: "#1a1a2e", fontSize: "24px", fontWeight: "800" }}>Edit Contact</h2>
               ...
               <form onSubmit={handleSubmit}>
                ...
                <div style={{ marginBottom: "16px" }}>
                     <label style={{ fontSize: "13px", fontWeight: "600", color: "#555", display: "block", marginBottom: "8px" }}>Phone number*</label> 
                     <input type="text" placeholder="0712345678" value={phone} onChange={(e) => setPhone(e.target.value)} style={{ width: "100%", padding: "14px 16px", borderRadius: "12px", border: "2px solid #e8e8e8", fontSize: "14px", outline: "none", boxSizing: "border-box" }} />  
                </div>
                ...
                <button onClick={handleSubmit} disabled={loading} style={{ width: "100%", padding: "16px", background: loading ? "#ccc" : "linear-gradient(135deg, #667eea, #764ba2)", colr: "white", border: "none", borderRadius: "14px", fontSize: "16px", fontWeight: "700", cursor: loading ? "not-allowed" : "pointer", boxShadow: "0 8px 25px rgba(102,126,234,0.5)" }}>
